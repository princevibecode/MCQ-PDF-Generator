        ...
        for _, row in group.iterrows():
            if is_bilingual:
                ...
                qt1 = str(row[f"{p1}_question_text"]) + (
                    f" &nbsp;<span style='color:#6B7280; font-size:0.9em;'><b>[{tag1}]</b></span>" if str(
                        tag1).strip() else "")
                qt2 = str(row[f"{p2}_question_text"]) + (
                    f" &nbsp;<span style='color:#6B7280; font-size:0.9em;'><b>[{tag2}]</b></span>" if str(
                        tag2).strip() else "")
                ...
                qt = str(row.get("question_text", "")) + (
                    f" &nbsp;<span style='color:#6B7280; font-size:0.9em;'><b>[{tag}]</b></span>" if str(
                        tag).strip() else "")
...
def clean_html_content(text):
    ...
    def replace_math(match):
        encoded = urllib.parse.quote(match.group(1).strip())
        return f'<img src="https://latex.codecogs.com/svg.image?{encoded}" style="vertical-align: middle; height: 1.8em; border: none; margin: 0 2px;" />'
...
with st.sidebar:
    st.header("⚙️ Promotion Settings")
    ...
    if promo_tier == "With Promotions":
        promo_level = st.radio("Promotion Level", [
            "Basic With Only Header",
            "Basic With Header & Footer Both",
            "Advance With Only Header",
            "Advance With Header & Footer Both"
        ])
    ...
    selected_color = st.color_picker("Primary Color (Chapters & Index)", "#00d1ff")
